<template>
  ...
  <div>
    <!--  -->
    <div class="show-model fixed inset-0 z-10"></div>
    <!--  -->
    <div class="fixed flex items-center justify-center inset-0 z-50">
      <!--  -->
      <div class="bg-white lg-col-6 md-col-7 sm-col-10 transition-all space-y-4 border border-gray-300 rounded-lg p-6 shadow-[0_2px_22px_-4px_rgba(93,96,127,0.2)]">
        <!--  -->
        <div class="mb-4">
          <h3 class="text-gray-800 text-3xl font-extrabold">{{ translate[$store.state.CURRENT_LANGUAGE].reset_password }}</h3>
          <p class="text-gray-500 text-sm mt-4 leading-relaxed">{{ translate[$store.state.CURRENT_LANGUAGE].reset_password_paragraph }}</p>
        </div>
        <!--  -->
        ...
        <div>
          <label class="text-gray-800 text-sm mb-2 block">
            {{ translate[$store.state.CURRENT_LANGUAGE].password }}
            <strong style="color:red">*</strong>
          </label>
          <div class="relative flex items-center">
            <input name="password" type="password" required class="w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-lg outline-blue-600" :placeholder="translate[$store.state.CURRENT_LANGUAGE].password_placeholder" v-model="rest.password"/>
        ...
        <div>
          <label class="text-gray-800 text-sm mb-2 block">
            {{ translate[$store.state.CURRENT_LANGUAGE].password_confirm }}
            <strong style="color:red">*</strong>
          </label>
          <div class="relative flex items-center">
            <input name="confirm_password" type="password" required class="w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-lg outline-blue-600" :placeholder="translate[$store.state.CURRENT_LANGUAGE].password_confirm_placeholder" v-model="rest.passwordConfirm"/>
            <svg class="w-[18px] h-[18px] absolute right-4"  xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 24 24" fill="none">
        ...
        <!--  -->
        <div class="!mt-6">
          <button type="button" class="bg-primary w-full shadow-xl py-3 px-4 text-sm tracking-wide rounded-lg font-semibold text-white hover:opacity-80 focus:outline-none" @click="updatePassword()">
            <span>{{ translate[$store.state.CURRENT_LANGUAGE].save }}</span>
          </button>
        </div>
        <!--  -->
      </div>
...
export default {
  ...
  methods: {
    async updatePassword() {
      if (this.rest.password == this.rest.passwordConfirm) {
        try {
          var object = {
          ...
          await this.$http.post(
            `/customers/update?id=${this.data.storeId}`,
            object,
            { headers: { "x-auth-token": this.data.token } }
          );
          location.href = location.origin + "/checkout2/orders";
        } catch (error) {
